...
import { buildTyreEvidences } from './evidence-builder';
...
export function TyreHypothesesTab({ data }: { data: TyreLabResponse }) {
  const totalWearSignals = data.dataset.filter(row => row.wear !== null).length;
  ...
  const compoundSignals = new Set(data.dataset.map(row => row.compound).filter(Boolean) as string[]).size;
  const weatherSignals = new Set(data.dataset.map(row => row.weather).filter(Boolean) as string[]).size;
  ...
  const hypotheses = [
    {
      title: 'Temperaturas maiores parecem aumentar o desgaste',
      description: 'Hipótese inicial construída a partir dos registros observados no Tyre Lab.',
      observations: temperatureSignals,
      status: totalWearSignals > 0 ? (temperatureSignals > 10 ? 'Suporte moderado' : 'Em análise') : 'Sem dados',
      confidence: totalWearSignals > 0 ? Math.min(85, 40 + Math.round((temperatureSignals / Math.max(1, data.dataset.length)) * 100)) : 0,
      supportedBy: [byId('E2'), byId('E3')].filter(Boolean) as Evidence[],
    },
    {
      title: 'Algumas pistas apresentam maior desgaste',
      description: 'O padrão pode variar conforme a pista e a temperatura capturada.',
      observations: trackSignals,
      status: trackSignals > 2 ? 'Suporte moderado' : 'Em análise',
      confidence: trackSignals > 0 ? Math.min(78, 30 + trackSignals * 8) : 0,
      supportedBy: [byId('E2'), byId('E4')].filter(Boolean) as Evidence[],
    },
    {
      title: 'Existem diferenças entre compostos',
      description: 'A base já mostra compostos distintos com quantidades diferentes de observações.',
      observations: compoundSignals,
      status: compoundSignals > 1 ? 'Suporte moderado' : 'Em análise',
      confidence: compoundSignals > 0 ? Math.min(72, 25 + compoundSignals * 10) : 0,
      supportedBy: [byId('E1')].filter(Boolean) as Evidence[],
    },
    {
      title: 'Há dados suficientes para iniciar um modelo matemático',
      description: 'Ainda é uma hipótese de prontidão, não uma conclusão.',
      observations: weatherSignals,
      status: data.dataset.length > 20 ? 'Bom suporte' : data.dataset.length > 8 ? 'Suporte moderado' : 'Sem dados',
      confidence: data.dataset.length > 20 ? 80 : data.dataset.length > 8 ? 55 : 0,
      supportedBy: [byId('E1'), byId('E2'), byId('E3'), byId('E4')].filter(Boolean) as Evidence[],
    },
  ];
