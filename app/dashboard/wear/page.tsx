...
import { 
  ...
  CheckCircle2, Trash2, User, Eraser
} from 'lucide-react';
...
const CAR_PARTS = [
  ...
  { id: 'asa_diant', label: 'ASA DIANTEIRA', icon: <Activity size={12} /> },
  { id: 'asa_tras', label: 'ASA TRASEIRA', icon: <Activity size={12} /> },
...
// --- MAPEAMENTO DE BANDEIRAS (ATUALIZADO E BLINDADO) ---
const TRACK_TO_FLAG: Record<string, string> = {
  "Adelaide": "au", "Ahvenisto": "fi", "Anderstorp": "se", "Austin": "us", "Avus": "de", 
  "A1-Ring": "at", "A1 Ring": "at", "Red Bull Ring": "at", "Oesterreichring": "at", // Blindagem Áustria
...
export default function WearPlanningPage() {
  ...
  // Configuração dos Atributos para o Header
  const driverStats = [
  ...
  // Ref para garantir salvamento ao sair (unmount)
  ...
  // --- FUNÇÃO AUXILIAR BLINDADA PARA BANDEIRAS ---
  const getFlagSrc = (trackName: string) => {
    if (!trackName) return '/flags/xx.png';
    const cleanName = trackName.trim(); 
    const code = TRACK_TO_FLAG[cleanName] || 
                 TRACK_TO_FLAG[cleanName.replace('-', ' ')] || 
                 'xx';
  ...
  const updateSeasonSlot = (index: number, field: string, value: any) => {
    ...
    setSeasonSlots(prev => prev.map((slot, i) => 
  ...
  const resetTrackSlot = (index: number) => {
    if (lockedSlots.includes(index)) return;
    if (!confirm(`Tem certeza que deseja limpar os dados de CTR e Teste para ${seasonSlots[index].name}?`)) {
      return;
    }
    setSeasonSlots(prev => prev.map((slot, i) => 
      i === index ? { ...slot, ctr: 0, testLaps: 0 } : slot
    ));
  };
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-[#050505]">
      ...
      <span className="text-zinc-500 font-mono text-xs tracking-[0.3em] uppercase tracking-tighter">Sincronizando com a Nuvem...</span>
    </div>
  );
  ...
  return (
    <div className="p-4 sm:p-8 space-y-6 text-slate-200 pb-40 bg-[#050505] min-h-screen">
      ...
      <div className="bg-zinc-900/40 backdrop-blur-2xl border border-white/5 p-6 rounded-[2.5rem] flex flex-col xl:flex-row justify-between items-center gap-6 shadow-2xl relative overflow-hidden">
            ...
            <div className="flex items-center gap-1 bg-black/40 px-6 py-2 rounded-2xl border border-white/5 shadow-inner backdrop-blur-sm">
                ...
                {driverStats.map((stat) => (
                    <div key={stat.label} className="flex flex-col items-center px-4 border-r border-white/5 last:border-0 group cursor-help relative">
                        <span className="text-[8px] text-zinc-500 font-black uppercase tracking-wider mb-0.5 group-hover:text-amber-500 transition-colors">
                            {stat.label}
                        </span>
                        ...
                        <div className="absolute -top-8 bg-black border border-white/10 px-2 py-1 rounded text-[10px] opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50">
                            {stat.full}
                        </div>
                    </div>
                ))}
        ...
        <div className="flex items-center gap-4 bg-black/60 px-6 py-3 rounded-2xl border border-white/5 font-mono z-10">
          <button 
            onClick={() => { 
              if(confirm("ATENÇÃO: Limpar nuvem? Isso resetará TODO o planejamento de todas as pistas.")) { 
                saveToCloud([], {}, []); 
                window.location.reload(); 
              } 
            }} 
            className="mr-2 p-2 text-zinc-600 hover:text-rose-500 transition-colors group relative"
            title="Resetar TUDO (Reset de Fábrica)"
          >
            <Trash2 size={16}/>
            <span className="absolute -bottom-8 right-0 bg-rose-950 text-rose-200 text-[9px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 whitespace-nowrap border border-rose-500/20">Reset Global</span>
          </button>
      ...
      {/* GRID DE TELEMETRIA */}
      <div className="bg-zinc-900/20 border border-white/5 rounded-[3rem] overflow-hidden backdrop-blur-md shadow-3xl">
                      ...
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <div className={`flex items-center justify-between gap-3 w-full px-4 py-2 rounded-xl border transition-all ${isLocked ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-white/5 border-white/5'}`}>
                          <div className="flex items-center gap-2 truncate">
                             ...
                             <span className={`text-[10px] font-black uppercase italic truncate ${isLocked ? 'text-emerald-400' : 'text-zinc-300'}`}>
                               #{i + 1} {slot.name}
                             </span>
                          </div>
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <button 
                                ...
                                className={`p-1.5 rounded-lg transition-all ${isLocked ? 'text-emerald-400 bg-emerald-500/20' : 'text-zinc-600 hover:text-white'}`}
                                title={isLocked ? "Destrancar Pista" : "Trancar Pista (Protege dados)"}
                            >
                        ...
                        <div className={`flex items-center gap-3 ${isLocked ? 'opacity-30 pointer-events-none' : ''}`}>
                          ...
                          <div className="flex flex-col items-center">
                              ...
                              <button 
                                  onClick={() => updateSeasonSlot(i, 'testEnabled', !slot.testEnabled)}
                                  className={`h-8 px-3 rounded-lg border text-[10px] font-black transition-all ${slot.testEnabled ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-rose-500/10 border-rose-500/30 text-rose-400'}`}
                              >
                                  {slot.testEnabled ? 'ACTIVE' : 'OFF'}
                              </button>
                          </div>
                        </div>
              ...
              {CAR_PARTS.map((part, pIdx) => (
                <tr key={part.id} className="group hover:bg-emerald-500/[0.02] transition-colors border-b border-white/5">
                  ...
                  {seasonSlots.map((_, sIdx) => {
                    ...
                    const baseTab = (sIdx + 1) * 100 + (pIdx * 2);
                    ...
                    return (
                      <td key={sIdx} className={`p-3 border-r border-white/5 transition-all ${isLocked ? 'grayscale-[0.5] opacity-80' : ''}`}>
                        <div className={`grid grid-cols-4 items-center gap-2 bg-black/40 p-2.5 rounded-2xl border transition-all ${isLocked ? 'border-emerald-500/10' : 'border-white/[0.03]'}`}>
                          <input 
                              disabled={isLocked} tabIndex={baseTab + 1}
                              type="text" placeholder="6" value={override.lvl || ""} 
                              onChange={(e) => updateOverride(part.id, sIdx, 'lvl', e.target.value.replace(/\D/g, ''))}
                              className={`w-full bg-zinc-800/30 text-[11px] text-center font-mono rounded-md py-1 outline-none ${isLocked ? 'text-zinc-600' : 'text-white focus:ring-1 ring-amber-500/50'}`} 
                          />
                          <input 
                              disabled={isLocked} tabIndex={baseTab + 2}
                              type="text" 
                              // CORREÇÃO AQUI: Convertendo para String
                              ...
                              className={`w-full bg-zinc-800/30 text-[11px] text-center font-mono rounded-md py-1 outline-none ${isLocked ? 'text-zinc-600' : (override.start !== undefined ? 'text-amber-400' : 'text-zinc-500')}`} 
                          />
                          ...
                          <div className={`relative flex items-center justify-center h-8 rounded-lg font-black text-xs border ${
                            isLocked ? 'bg-emerald-900/20 border-emerald-500/20 text-emerald-600' :
                            finalVal > 90 ? 'bg-rose-500/20 text-rose-500 border-rose-500/30' : 
                            finalVal > 70 ? 'bg-amber-500/20 text-amber-400 border-amber-500/30' : 
...
function InputHeader({ label, value, onChange, color }: any) {
  return (
    <div className="flex flex-col items-center">
      ...
      <input 
        type="text" value={value} onChange={(e) => onChange(e.target.value.replace(/\D/g, ''))}
