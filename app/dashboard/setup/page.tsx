...
import { 
  Loader2, RefreshCw, Zap, Gauge, Thermometer, Flag, 
  AlertTriangle, CloudSun, Target, Activity, ShieldAlert
} from 'lucide-react';
...
export default function SetupPage() {
  ...
  if (!initialLoaded) return <div className="flex h-screen items-center justify-center bg-[#050507]"><Loader2 className="animate-spin text-indigo-500" size={48} /></div>;
        ...
        <header className="sticky top-0 z-40 bg-black/40 backdrop-blur-xl border-b border-white/5 px-6 py-4">
            <div className="max-w-[1600px] mx-auto flex justify-between items-center">
                <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-indigo-600 rounded flex items-center justify-center shadow-lg"><Activity className="text-white" size={20} /></div>
                ...
                <div className="flex items-center gap-4 bg-black/40 p-1 rounded-lg border border-white/5">
                    ...
                    <div className="relative min-w-[250px]">
                        <select value={track} onChange={(e) => updateTrack(e.target.value)} className="w-full h-9 bg-transparent text-white font-bold text-[10px] outline-none appearance-none px-4">
                            {tracks.map((t, i) => <option key={i} value={t} className="bg-slate-900">{t.toUpperCase()}</option>)}
                        </select>
        ...
        <main className="max-w-[1600px] mx-auto px-6 py-8 grid grid-cols-1 xl:grid-cols-12 gap-8">
             <div className="xl:col-span-7 space-y-6">
                <section className="bg-white/[0.02] border border-white/5 rounded-2xl p-8 backdrop-blur-sm">
                    <h2 className="text-[10px] font-black text-white uppercase tracking-[0.3em] mb-10 flex items-center gap-3"><CloudSun className="text-indigo-400" size={16} /> Meteorologia_Ativa</h2>
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                                ...
                                <div className="p-4 space-y-1">
                                    ...
                                    {['asaDianteira', 'asaTraseira', 'motor', 'freios', 'cambio', 'suspensao'].map((part) => (
                                        <div key={part} className="grid grid-cols-4 items-center bg-black/20 py-2.5 px-4 rounded border border-white/5">
                                            ...
                                            <span className="text-center text-[11px] text-white/50">{resultado[part]?.q1 || '---'}</span>
                                            <span className="text-center text-[11px] text-white/50">{resultado[part]?.q2 || '---'}</span>
                                            <span className="text-center text-sm font-black text-white">{resultado[part]?.race || '---'}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-6 backdrop-blur-sm">
                                <div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4">
                                    <h2 className="text-[10px] font-black text-white uppercase tracking-[0.2em] flex items-center gap-2"><ShieldAlert size={14} className="text-rose-500" /> Desgaste_Geral_Estimado</h2>
                                    <div className="bg-black/50 px-3 py-1 rounded border border-white/10 flex items-center gap-3">
                                        ...
                                        <input type="number" value={desgasteModifier} onChange={(e) => updateDesgasteModifier(Number(e.target.value))} className="w-8 bg-transparent text-white text-center text-xs font-black outline-none" />
                                    </div>
                                </div>
                                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-3 custom-scrollbar">
                                    {COMPONENTS.map((part) => {
                                        ...
                                        if (!d) return null;
                                        const isCritical = d.end > 85;
                                        return (
                                            <div key={part.id} className="space-y-1.5">
                                                <div className="flex justify-between text-[8px] font-black uppercase tracking-tighter">
                                                    <span className="text-slate-500">{part.label}</span>
                                                    <span className="text-slate-300">{d.start}% → <span className={isCritical ? 'text-rose-500 font-black animate-pulse' : 'text-white'}>{d.end.toFixed(1)}%</span></span>
                                                </div>
                                                <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden flex border border-white/5">
                                                    <div className="h-full bg-slate-700" style={{ width: `${d.start}%` }}></div>
                                                    <div className={`h-full ${isCritical ? 'bg-rose-500 shadow-[0_0_8px_#f43f5e]' : 'bg-indigo-500'}`} style={{ width: `${Math.max(0, d.end - d.start)}%` }}></div>
                                                </div>
                                            </div>
                                        )
                                    })}
...
function SessionGroup({ title, children }: any) { return ( <div className="space-y-4"> <h3 className="text-[8px] font-black text-slate-500 uppercase tracking-widest border-l-2 border-indigo-500/30 pl-3">{title}</h3> <div className="space-y-4">{children}</div> </div> ) }
function HUDInput({ value, name, onChange, label }: any) { return ( <div className="bg-black/40 border border-white/5 rounded-lg p-3 group hover:border-indigo-500/40 transition-all"> <label className="block text-[7px] font-black text-slate-600 uppercase mb-1 tracking-widest">{label}</label> <div className="flex items-center justify-between"> <Thermometer size={14} className="text-indigo-400/50" /> <input type="number" name={name} value={value || ''} onChange={onChange} className="bg-transparent text-right text-white font-black text-sm outline-none w-full" /> <span className="text-[9px] text-slate-600 font-bold ml-2">°C</span> </div> </div> ) }
function WeatherSwitch({ name, value, onChange }: any) { const isDry = value === 'Dry'; return ( <div className="flex bg-black p-1 rounded-lg border border-white/5"> <button onClick={() => onChange({ target: { name, value: 'Dry' } } as any)} className={`flex-1 py-2 rounded text-[8px] font-black uppercase transition-all ${isDry ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-600 hover:text-slate-500'}`}>☀️ Seco</button> <button onClick={() => onChange({ target: { name, value: 'Wet' } } as any)} className={`flex-1 py-2 rounded text-[8px] font-black uppercase transition-all ${!isDry ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-600 hover:text-slate-500'}`}>🌧️ Chuva</button> </div> ) }