...
import { ChevronLeft, Activity, Brain, Flame, Grid2x2, Layers3, Lightbulb, Scale, Sigma } from 'lucide-react';
import { TyreDataTab } from '@/app/components/research/TyreDataTab';
import { TyreDiscoveryTab } from '@/app/components/research/TyreDiscoveryTab';
import { TyreCorrelationTab } from '@/app/components/research/TyreCorrelationTab';
import { TyreHypothesesTab } from '@/app/components/research/TyreHypothesesTab';
...
import type { TyreLabResponse, TyreObservationRow } from '@/app/components/research/tyre-types';
...
export default function TyreLabPage() {
  const [userId, setUserId] = useState<string | null>(null);
  const [isLoadingAuth, setIsLoadingAuth] = useState(true);
  const [isLoadingData, setIsLoadingData] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [data, setData] = useState<TyreLabResponse | null>(null);
  const [activeTab, setActiveTab] = useState<TabId>('data');
  const [query, setQuery] = useState('');
  const [trackFilter, setTrackFilter] = useState('');
  const [sourceFilter, setSourceFilter] = useState('');
  const [researchQuery, setResearchQuery] = useState<ResearchQuery>(EMPTY_RESEARCH_QUERY);
  const [selectedRow, setSelectedRow] = useState<TyreObservationRow | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        const response = await fetch('/api/admin/research/tyres', {
          headers: {
        ...
        if (!response.ok || !payload.success) {
          throw new Error(payload.error || 'Erro ao carregar Tyre Lab');
        }

        setData(payload);
      } catch (err) {
        console.error(err);
        setError(err instanceof Error ? err.message : 'Erro ao carregar Tyre Lab');
      } finally {
  ...
  const filteredDataset = useMemo(() => {
    ...
    return data.dataset.filter(row => {
      const matchesQuery = !query || [row.track, row.weather, row.compound, row.stint, row.endpoint, row.source, row.season !== null && row.race !== null ? `S${row.season} · R${row.race}` : '']
      ...
      const matchesCompound = !researchQuery.compound || row.compound === researchQuery.compound;
      ...
      const matchesWearRange = (!researchQuery.onlyWear || row.wear !== null) && matchesNumericRange(row.wear, researchQuery.wearMin, researchQuery.wearMax);
      const matchesOnlyCompound = !researchQuery.onlyCompound || row.compound !== null;
      return matchesQuery
        && matchesTrack
        && matchesSource
  ...
  if (!userId) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[radial-gradient(circle_at_top,_#f7fbf9,_#edf3f0_40%,_#e4ebe7_100%)] text-slate-800 p-6">
        <div className="max-w-lg rounded-[2rem] border border-white/70 bg-white/90 p-8 shadow-[0_20px_60px_rgba(15,23,42,0.08)] backdrop-blur-xl">
          <p className="text-[10px] font-black uppercase tracking-[0.35em] text-slate-500">Tyre Research Lab</p>
  ...
  const tabContent = (() => {
    if (!data) return null;
    switch (activeTab) {
      case 'data': return <TyreDataTab overview={overview} header={header} filteredDataset={filteredDataset} trackExplorer={data.trackExplorer} uniqueTracks={uniqueTracks} uniqueSources={uniqueSources} query={query} setQuery={setQuery} trackFilter={trackFilter} setTrackFilter={setTrackFilter} sourceFilter={sourceFilter} setSourceFilter={setSourceFilter} selectedRow={selectedRow} setSelectedRow={setSelectedRow} data={data} queryState={researchQuery} setQueryState={setResearchQuery} onClearQuery={handleClearResearchQuery} onSaveQuery={handleSaveResearchQuery} onExportQuery={handleExportResearchQuery} />;
      case 'discovery': return <TyreDiscoveryTab data={data} />;
      case 'variables': return <ResearchVariablesTab variables={data.knowledgePanel.variableCatalog} title="Catálogo científico de variáveis observadas" description="Esta aba organiza as variáveis encontradas nos snapshots em vez de mostrar apenas linhas." />;
      case 'correlations': return <TyreCorrelationTab data={data} />;
      case 'hypotheses': return <TyreHypothesesTab data={data} />;
      case 'evidence': return <ResearchEvidenceTab variables={data.knowledgePanel.variableCatalog} datasetLength={data.dataset.length} dataset={data.dataset} lab="tyre" />;
      case 'insights': return <ResearchInsightsTab variables={data.knowledgePanel.variableCatalog} datasetLength={data.dataset.length} dataset={data.dataset} lab="tyre" />;
      default: return null;
    }
  })();
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_#f7fbf9,_#edf3f0_40%,_#e4ebe7_100%)] text-slate-800">
      <div className="mx-auto w-full max-w-[1800px] px-4 py-6 md:px-6 lg:px-8 pb-24">
        <header className="overflow-hidden rounded-[2rem] border border-white/70 bg-white/80 backdrop-blur-xl shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
          <div className="flex flex-col gap-6 p-6 md:p-8 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <h1 className="mt-3 text-4xl md:text-5xl font-black tracking-tight text-slate-950">Tyre Research Lab</h1>
              <p className="mt-3 max-w-2xl text-sm md:text-base text-slate-600 leading-relaxed">
                Centro de pesquisa para engenharia reversa do desgaste de pneus.
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 border-t border-slate-200/70 p-6 md:grid-cols-4 lg:grid-cols-4">
            <div className="rounded-2xl border border-slate-200 bg-slate-50/90 p-4 col-span-2 md:col-span-1 lg:col-span-1">
